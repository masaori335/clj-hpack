...
(deftest D.2.1
  ...
  (reset! cursor 0)
  (testing ""
    (is (= (encode [["custom-key" "custom-header"]])
...
(deftest D.3.1
  "First Request"
  (testing ""
    (is (= (encode [[":method" "GET"] [":scheme" "http"] [":path" "/"] [":authority" "www.example.com"]])
...
(deftest D.3.2
  ...
  (testing ""
    (is (= (encode [[":method" "GET"] [":scheme" "http"] [":path" "/"] [":authority" "www.example.com"]])
           [0x82 0x86 0x84 0x41 0x0f 0x77 0x77 0x77 0x2e 0x65 0x78 0x61 0x6d 0x70 0x6c 0x65 0x2e 0x63 0x6f 0x6d]))
    (is (= (encode [[":method" "GET"] [":scheme" "http"] [":path" "/"] [":authority" "www.example.com"] ["cache-control" "no-cache"]])
...
(deftest D.3.3
  ...
  (testing ""
    ...
    (is (= (encode [[":method" "GET"] [":scheme" "http"] [":path" "/"] [":authority" "www.example.com"] ["cache-control" "no-cache"]])
           [0x82 0x86 0x84 0xbe 0x58 0x08 0x6e 0x6f 0x2d 0x63 0x61 0x63 0x68 0x65]))
    (is (= (encode [[":method" "GET"] [":scheme" "https"] [":path" "/index.html"] [":authority" "www.example.com"] ["custom-key" "custom-value"]])
