...
(deftest D.1.1
  ...
  (testing ""
    (is (= (integer-representation 10 5) '(2r01010)))))

(deftest D.1.2
  "Example 2: Encoding 1337 Using a 5-bit Prefix"
  (testing ""
    (is (= (integer-representation 1337 5) '(2r00011111 2r10011010 2r00001010)))))

(deftest D.1.3
  "Example 3: Encoding 42 Starting at an Octet Boundary"
  (testing ""
    (is (= (integer-representation 42 8) '(2r00101010)))))
...
(deftest D.2.1
  ...
  (testing ""
    (let [table (HeaderTable. '())]
      (is (= (literal-header-field-with-icremental-indexing table ["custom-key" "custom-header"])
...
(deftest D.2.2
  ...
  (let [table (HeaderTable. '())]
    (testing ""
      (is (= (literal-header-field-without-indexing table [":path" "/sample/path"])
...
(deftest D.2.3
  ...
  (let [table (HeaderTable. '())]
    (testing ""
      (is (= (literal-header-field-never-indexed table ["password" "secret"])
...
(deftest D.2.4
  ...
  (let [table (HeaderTable. '())]
    (testing ""
      (is (= (indexed-header-field-representation table [":method" "GET"])
             '(0x82))))))
